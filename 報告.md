# HW1 AIoT 系統實作專題報告：雙軌架構下之邊緣端感測與雲端展示系統 --- 4112064205 電資三 鄭弘翊
...
## 3. 實驗結果與展示 (Results & Demonstrations)
...
>
> ![ESP32實機](./picture/esp32實機部份.png "ESP32實機部份")
...
為確保後續專題重現性，本附錄摘錄自專案主說明檔 (`README.md`)，提供系統整體架構配置與執行步驟。

### A.1 專案目錄配置

```text
d:\IOT_HW1\
├── .agent/              # AI 代理人 Skill 設定區
├── frontend_vercel/     # 【雲端軌道】純前端模擬 Live Demo (無資料庫依賴)
├── frontend/            # 【地端軌道】銜接 FastApi 伺服器的全功能 Dashboard
├── backend/
│   ├── mock_db/         # 產生模擬 SQLite 測試資料的獨立專案
│   └── wifi_iot/        # 接收 ESP32 真實數據的 FastAPI 伺服器
├── edge/
│   ├── DHT11/           # 【測試】讀取 DHT11 溫濕度並印出於終端
│   ├── DHT11_WIFI/      # 【正式】讀取 DHT11 數據並 WiFi POST 至地端後端
│   └── LED/             # 【測試】ESP32 GPIO LED 閃爍測試
├── 任務紀錄.md            # 開發日誌 (Development Log)
├── 報告.md                # 專題結案報告
└── README.md            # 本專案總體說明文件
```
...
**軌道一：雲端 Live Demo (Vercel)**
進入 `frontend_vercel/` 後，直接將該存放庫綁定 Vercel 即可部署，無後端與資料庫之環境依賴。

**軌道二：地端全系統串接 (真實硬體測試)**

1. **啟動後端：** 於 `backend/wifi_iot/` 執行 FastAPI 伺服器：
   `uv run -m uvicorn main:app --host 0.0.0.0 --port 8000 --reload`
2. **尋找端點 IP：** 再開一終端機輸入 `ipconfig`，找出連上手機 WiFi 熱點之區網 IP 位址。
3. **啟動前端：** 於 `frontend/` 執行 `npm run dev` 開啟監控面板。
4. **燒錄韌體：** 開啟 `edge/DHT11_WIFI/src/main.cpp`，填寫對應之手機 WiFi 憑證與剛才找到的 API 位址，將程式燒錄至開發板後即可完成全系統閉環。
